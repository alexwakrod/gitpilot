"""Daemon lifecycle: manages file watchers, AI committer, and intelligence engine."""
...
class DaemonLifecycle:
    """Manages watcher threads, AI committer, executor, and intelligence settings."""
    ...
    def start(self):
        """Start the file watcher and background tasks. Always starts the observer thread."""
        logger.info("Starting daemon lifecycle")

        # Always create the watcher, even if no projects yet – projects can be added later.
        ...
        # Load existing active projects and add them to the watcher
    ...
    def add_project(self, project_id: int, path: str) -> None:
        """Add a project to the running watcher."""
        if self.watcher:
    ...
    def remove_project(self, project_id: int, path: str) -> None:
        """Remove a project from the running watcher."""
        if self.watcher:
