"""Fast tests for daemon server entrypoint, lifecycle, and CLI startup (no threads)."""
...
class TestDaemonServerFast:
    ...
    def test_setup_logging_no_crash(self, tmp_path):
        setup_logging(tmp_path / "logs")


class TestDaemonLifecycleFast:
    ...
    def test_verify_global_git_config_ok(self, monkeypatch):
        lc = DaemonLifecycle({})
